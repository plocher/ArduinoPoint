...
class IOCollection(SimpleNamedTypedList):
    ...
    def towiki(self):
        heading  = '<table border=1 style="font-family: monospace;width: 80%;">\n<tr>\n    <th colspan="{}"> {} for {}</th>\n</tr>\n'.format(self.bits + 1, self.mytype, self.cpname)
        tBegin   ='\n'
        prepend  = '<tr>\n    <th> {} </td> <td align="center">'.format(self.units)
        prepend2 = ' </th> <td align="center"> '
        postpend = '</td>\n</tr>\n'
        devname  = '<tr>\n    <th> {:<2}   </th>  '
        prepend3 = ' <td align="center"> '
        prepend4 = ' </td> <td align="center"> '
        postpend2= '</td>\n</tr>\n'
        tEnd     = '</table>\n'
        fmt8     = '{:<8}'
...
class ExpanderList(IOCollection):
    ...
    def towiki(self):
        ...
        tBegin   ='<tr>\n    <th> Expander </th><th> Type </th><th> I2C Address </th><th> I/O Map<br \>bit{} ... (1=input, 0=output) ... bit0</th></tr>\n'.format(self.bits - 1)
        devname  = '<tr>\n    <th> {:<2}   </th>  '
        prepend3 = ' 0x{:02x} '
        prepend4 = ' -na- '
        tEnd     = '</table>\n'
        fmt8     = ' <th> {type}</th> <td align="center"> {a} </td> <td align="center"> {init}</td>'
    ...
    def toDetailedWiki(self):
        ...
        def header(output, size):
            output.write('<tr>\n    <th>  # ')
            output.write(    ' </th><th>  Device\n')
            output.write(    ' </th><th>  i2c\n')
            output.write(    ' </th><th>  Bit\n')
            output.write(    ' </th><th>  Signal\n')
            output.write(    ' </th><th>  Bit\n')
            output.write(    ' </th><th>  Signal\n')
            if (int(size) > 8):
                output.write(    ' </th><th>  Bit\n')
        ...
        output.write('<tr> <th colspan="{span}" {style}"> {name} Wiring for {cp} </th> </tr>\n'.format(span=colspan,
        ...
        for key, val in self.children.items():
            ...
            output.write('<tr>\n')
            prefix='    <th rowspan=4> {num} </th><th rowspan=4>  {name} </th><th rowspan=4> {addr} </th>\n'.format(
                num=val.device,
                name=i2ctype,
                addr=a)
...
class SW_Route(Appliance):
    ...
    def toPosition(self):
        pos = 'SwitchDevice::UNKNOWN'
        if self.position == 'R':
            pos = 'SwitchDevice::REVERSE'
        elif self.position == 'N':
            pos = 'SwitchDevice::NORMAL'
        return pos
...
class ControlPoint:
    ...
    def toSignalCode(self, fieldunitname):
        s='\n'
        s += '    SignalDevice *s = NULL;\n'
        s += '    SignalHead   *h = NULL;\n'
        ...
        for fk, fv in cp.field.children.items():
            if fk == fieldunitname:
                for smname, sm in fv.signals.children.items():
                    ...
                    s += '\n'
                    s += '    // ===================================================================\n'
                    s += '    //  {} Signal {}: {}\n'.format(cp.name, smname, docstring)
                    s += '    // ===================================================================\n'
                    s += '\n'
                    s += '    s = (SignalDevice *)fieldUnit->get("{signame}");\n'.format(signame=signalFullName)
                    s += '\n'

                    if (siga.fleet is not None) and (siga.er is not None):
                        if (siga.fleet.lower() == 'true') and (siga.er.lower() == 'true'):
                            s += '    s->setStick(SignalDevice::FLEET_ER);\n'
                        elif (siga.fleet.lower() == 'true') and (siga.er.lower() != 'true'):
                            s += '    s->setStick(SignalDevice::FLEET);\n'
                        elif (siga.fleet.lower() == 'true') and (siga.er.lower() != 'true'):
                            s += '    s->setStick(SignalDevice::ER);\n'
                    for shname, sh in sm.children.items():
                        ...
                        #s += '\n'
                        s += '        // --------------------------------------------------------------\n'
                        s += '        //  {} {}\n'.format(mastFullName, docstring)
                        s += '        // --------------------------------------------------------------\n'
                        s += '        //\n'
                        s += '        h = new SignalHead_{headtype}("{headname}", '.format(headtype=sh.type, headname=mastFullName)
                        sep=''
                        for outname, out in iter(natsorted(sh.field.children.items())):
                            s += '{sep}{cpname}_{outname}F'.format(sep=sep, cpname=cp.name, outname=outname)
                            sep = ', '
                        s += ');\n'
                        s += '        s->addHead(h);\n'
                        ...
                        for rk, rv in heada.route.children.items():
                            ...
                            if rv.default_aspect is None:
                                s += '\t    // No default aspect provided'
                                for val, item in rv.children.items():
                                    s += '\t    h->addRouteDependency({routename:<15} {condition});\n'.format(
                                        routename='"{key}",'.format(key=rk),
                                        condition=item.toSignalCode(True))
                            else:
                                s += '\t    h->createRoute({routename:<15} {aspect});\n'.format(routename='"{key}",'.format(key=rk),
                                                                                                aspect=rv.default_aspect)
                                for val, item in rv.children.items():
                                    s += '\t    h->addRouteDependency({routename:<15} {condition});\n'.format(
                                        routename='"{key}",'.format(key=rk),
                                        condition=item.toSignalCode(False))
                        s += '\n'
        return s
    ...
    def walk(self, list):
        ...
        for k, v in list.items():
            s += cp.define(v.fullname, v.fieldvalue) + '\n'
        return s
    ...
    '''
    Generate text-tabled topmatter in a C++ // comment block
    '''
    def toheading(self, fieldname):
        output = StringIO()
        output.write('    Autogenerated Control Point code for\n')
        output.write('    ************************************\n')
        output.write('    {:^34}\n'.format(self.name + ' Field Unit'))
        output.write('    ************************************\n')
        ...
        if len(self.indications.children) > 0:
            output.write( self.indications.totext())
        output.write('\n')
        if len(self.controls.children) > 0:
            output.write( self.controls.totext())
        output.write('\n')
        ...
        for k, v in self.FieldunitList():
            if v.name == fieldname:
                output.write(v.expanders.totext())
                output.write('\n')
                output.write(v.expanders.toDetailedText())
                output.write('\n')
...
def write_template(args, dirname, fname):
    print("Template:  using {}".format(args.templatename))
    ...
    for line in lines:
        ...
        if True or cleanline != '':
            clean_result = clean_result + sep + line
            sep = "\n"
    clean_result = clean_result + '\n'
    write_sketch(d, f, clean_result)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--type',           dest="type", choices=['html', 'ctc', 'decoder', 'jmri', 'sketch'])
    ...
    if args.type == "html" \
        ...
        if not args.outfilename :
            ...
            if args.type == "decoder":
                ...
                b = ("{cp}").format(cp=myname)
            #d = "/Users/plocher/Documents/Dropbox/Arduino/Auto/" + b
        ...
        if args.type == 'html':
            if args.templatename is None:
                args.templatename = ["html.template.mako"]
            write_template(args, d, f)
        elif args.type == 'jmri':
            if args.templatename is None:
                args.templatename = ["jmri.template.mako"]
            write_template(args, d, f)
        elif args.type == 'decoder':
            if args.templatename is None:
                args.templatename = ["codelineDecoder.template.mako"]
            write_template(args, d, f)
        elif args.type == 'sketch':
            if args.templatename is None:
                args.templatename = ["fieldsketch.template.mako"]
            write_template(args, d, f)
        elif args.type == 'ctc':
            if args.templatename is None:
                args.templatename = ["ctcPanel.template.mako"]
            write_template(args, d, f)
